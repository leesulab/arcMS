...
`arcMS` can convert (HD)MS<sup>E</sup> data acquired with Waters UNIFI to tabular format for use in R or Python, with a small filesize when saved on disk. It is compatible with data containing ion mobility (HDMS<sup>E</sup>) or not (MS<sup>E</sup>). Conversion of mzML files is also supported (see `convert_mzml_to_parquet()`).
...
-   the [HDF5](https://www.hdfgroup.org/solutions/hdf5/) format, with fast access but larger filesize.
...
Also, check the `vignette("open-files")` for details on how converted files can be opened in R or Python, and the [full tutorial](articles/data-filtration-tutorial.html) on how to query, filter, aggregate data (e.g. to obtain chromatograms or spectra).
