...
```{r, include = FALSE}
library(arrow)
data = read_parquet("../../arcMS-dataviz/cal1-full-long.parquet")
```
...
```{r, include = FALSE}
data = open_dataset("../../arcMS-dataviz/cal1-full-long.parquet")
```
...
import pandas as pd
data = pd.read_parquet("../../arcMS-dataviz/cal1-full-long.parquet")
```
...
data = pq.ParquetDataset("../../arcMS-dataviz/cal1-full-long.parquet")
```
...
```{r}
library(arcMS)
dataset = create_sample_dataset("/media/julien/Data/SynologyDrive/code/R/parquet-MS/cal 1_replicate_1_with_metadata.parquet")  
```
