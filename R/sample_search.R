...
#' @param analysisid The identifier of the analysis for which sample results are to be retrieved.
...
#' sample_results <- sample_search(con_params, analysisid)
#'
#' @export

sample_search <- function(connection_params, analysisid) {
  url <- connection_apihosturl(connection_params)
  token <- connection_token(connection_params)

  url2 <- glue::glue("{url}/analyses({analysisid})/sampleresults")
  rg <- httr::GET(url2,
