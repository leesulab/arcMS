...
outputlist_to_df <- function(outputlist){
  data_all = data.frame(mz = double(),    # Create empty data frame
                        intensity = double(),
                        scan_size = double(),
                        rt = double(),
                        mslevel = double(),
  ...
  for (i in 1:length(outputlist)){
    data_all[i,]$intensity = list(outputlist[[i]]$Intensities)
    data_all[i,]$mz = list(outputlist[[i]]$MassSpectrum$Masses)
    ...
    data_all[i,]$mslevel = outputlist[[i]]$MassSpectrum$MSeMassSpectrum$EnergyLevel
